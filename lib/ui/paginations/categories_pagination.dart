...
import 'package:tepmare_warehouse_man_app/models/categories_model.dart';
import 'package:tepmare_warehouse_man_app/models/sites_model.dart';
import 'package:tepmare_warehouse_man_app/ui/components/category_item.dart';
...
class CategoriesPagination extends ConsumerWidget {
  CategoriesPagination({this.query});
  ...
  void getData(WidgetRef widgetRef) async {
    if (!isLastPage) {
      if (!isFetchingData) {
        ...
        CategoriesModel categoriesModel = await ApiManager.getCategories(
          page: page.toString(),
          limit: limit.toString(),
          query: query??"",
        );
        List<Category>? tempFetchedData = categoriesModel.categories;
