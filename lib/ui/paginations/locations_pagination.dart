...
import 'package:tepmare_warehouse_man_app/ui/components/location_item.dart';
...
class LocationsPagination extends ConsumerWidget {
  LocationsPagination({this.query,this.siteId});
  ...
  void getData(WidgetRef widgetRef) async {
    if (!isLastPage) {
      if (!isFetchingData) {
        ...
        LocationsModel locationsModel = await ApiManager.getLocations(
          page: page.toString(),
          limit: limit.toString(),
          query: query??"",
        );
        List<Location>? tempFetchedData = locationsModel.locations;
