...
import 'package:tepmare_warehouse_man_app/ui/components/shipment_item.dart';
...
class ShipmentsPagination extends ConsumerWidget {
  ShipmentsPagination({this.query});
  ...
  void getData(WidgetRef widgetRef) async {
    if (!isLastPage) {
      if (!isFetchingData) {
        ...
        ShipmentsModel shipmentsModel = await ApiManager.getShipments(
          page: page.toString(),
          limit: limit.toString(),
          query: query??"",
        );
        List<Shipment>? tempFetchedData = shipmentsModel.shipments;
